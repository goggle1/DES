...
int Hill2_EncryptText(const char *plainText, const char *keyStr, char *cipherText)
{
	...
	int plain_index = 0;
	for(plain_index=0; plain_index<plain_len; plain_index+=HILL_2)
	{
		int row_index = 0;
		for(row_index=0; row_index<HILL_2; row_index++)
		{
			int temp = 0;
			temp += hill2_encrypt_key[row_index][0] * (plainText[plain_index+0]);
			temp += hill2_encrypt_key[row_index][1] * (plainText[plain_index+1]);
			temp = temp%256;
...
int Hill2_DecryptText(const char *cipherText, const char *keyStr, char *plainText)
{
	...
	int cipher_index = 0;
	for(cipher_index=0; cipher_index<cipher_len; cipher_index+=HILL_2)
	{
		int row_index = 0;
		for(row_index=0; row_index<HILL_2; row_index++)
		{
			int temp = 0;
			temp += hill2_decrypt_key[row_index][0] * (cipherText[cipher_index+0]);
			temp += hill2_decrypt_key[row_index][1] * (cipherText[cipher_index+1]);
			temp = temp%256;
